...
def get_houses_index():
    ...
    # 　尝试从redis获取房屋首页幻灯片信息
    try:
        ret = redis_store.get('home_page_data').decode(encoding='utf-8')
    except Exception as e:
        current_app.logger.error(e)
        ret = None
...
def get_houses_list():
    ...
    try:
        ...
        redis_key = 'houses_%s_%s_%s_%s_%s' % (area_id, start_date_str, end_date_str, sort_key, page)
    ...
    if page <= total_page:
        try:
            ...
            redis_store.setex(redis_key, constants.HOME_PAGE_DATA_REDIS_EXPIRES, resp_json)
        except Exception as e:
